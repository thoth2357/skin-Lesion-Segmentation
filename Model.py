...
class Fcn_Network:
    ...
    def model_init(self,x_train,y_train,x_val,y_val, loss:List, metrics:List) -> tuple(keras.models.Model, keras.models.Model):
    # Convolution Layers (BatchNorm after non-linear activation)
        ...
        model.compile(optimizer= Adam(lr = self.LR), loss= loss
                    , metrics=metrics)

        hist = model.fit(x_train, y_train, epochs= self.EPOCHS, batch_size= 18, validation_data= (x_val, y_val), verbose=1)
